...
invoices are still unpaid* — and it searches, reads what it finds, and writes an
answer citing every document it used. The searches, reads and surveys appear as
they happen, so a long run is watchable rather than a spinner.
