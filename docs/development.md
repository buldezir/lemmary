...
The binary can be built with a private *edition* compiled in: extra routes,
extra pipeline stages, extra pages. This repository contains no such edition —
the seam exists so one can live in a fork that only ever **adds** files, and
therefore never has a merge conflict when it pulls from here.

One build argument selects both halves:

```bash
docker build --build-arg EDITION=cloud -t lemmary:cloud .
```

`EDITION=cloud` becomes the Go build tag `lemmary_cloud` and the frontend module
alias `@ext` → `src/ext-cloud`. Passing it to both stages from one argument is
deliberate: an image with the frontend half and not the backend half would serve
pages whose API routes do not exist.

### What an edition can add

| Field of [`ext.Edition`](../backend/internal/ext/ext.go) | Effect |
| --- | --- |
| `Steps` | pipeline stages; a stage whose `Name()` matches a built-in **replaces** it |
| `StepPlans` | rewrite the default step list for newly uploaded documents (never an explicitly requested list) |
| `DecorateSnapshot` | replace any OCR/AI client on every settings reload |
| `Register` | routes, hooks, cron jobs and CLI subcommands, bound after every core one |

The frontend counterpart is [`Edition`](../frontend/src/lib/edition.ts): extra
routes and extra entries in the "More" menu.

### Where the seam is, and where it is not

`appwire.Register` is the only backend wiring point an edition attaches to.
Nothing hangs off `backend/main.go` on purpose — it is the file that changes
most, so a fork that edited it would conflict on every merge.

### Adding an edition to a fork

Add files; edit none:

```text
backend/internal/appwire/edition_cloud.go   //go:build lemmary_cloud
backend/internal/cloud/                     the edition's own packages
backend/cloudmigrations/                    migrations, timestamped after core's
frontend/src/ext-cloud/index.ts             exports `edition`
```

`edition_oss.go` carries negative build tags, so a new edition tag must be added
to its constraint too. Two editions in one binary is a compile error by design.

Health metric for the fork: `git diff upstream/main --stat` should list added
files and nothing else. A modified core file means a missing seam — add the seam
upstream rather than carrying the edit.

### The `exttest` edition

`EDITION=exttest` builds a throwaway edition that ships in this repository
([`edition_exttest.go`](../backend/internal/appwire/edition_exttest.go),
[`src/ext-exttest/`](../frontend/src/ext-exttest)). It exercises every field
above so that a change which stops wiring one of them fails here rather than in
a fork's CI weeks later:

```bash
cd backend && go test -tags lemmary_exttest ./e2e/ -run Edition -count=1
```

`./scripts/test-all.sh` runs it.
