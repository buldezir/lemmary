...
- The whole archive is resident in RAM while unlocked, so RAM bounds how large an
  ...
  stored in `data.db`, and again for the index built from them. Budget roughly
  `chunks x dimensions x 4 bytes` for each — a 1536-dimension model over an
  archive of 10,000 documents at ~5 passages each is about 300 MB of vectors plus
  a similar index. A model with 1024 dimensions or fewer costs proportionally
  less, which is the reason to prefer one here even where the accuracy of a
  larger model would be free. Size the tmpfs in `docker-compose.encrypted.yml`
