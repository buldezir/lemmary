...
- **Scan every (minutes)** — default 5. Saving re-schedules the `dir_ingest` cron (visible in PocketBase Admin → Settings → Crons), so a change applies without a restart; from 60 up the schedule becomes hourly steps.
- **Delete the original file after it is consumed** — off by default. Off, files stay in place and the checksum duplicate check is what keeps them from being imported twice: a file whose content is already in the library is skipped, and remembered as skipped for the life of the process so it is not re-hashed every scan. On, a file is removed once its document exists, and a file that turns out to be a duplicate is removed as well. A file the pipeline refuses (wrong content for its extension, over a per-file limit) is never deleted; it is logged and skipped. Reaching an instance-wide limit stops the scan until the next interval.
