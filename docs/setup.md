...
| `MCP_ENABLED` | unset (off) | Set to `1`/`true` to serve a read-only [Model Context Protocol](/mcp) endpoint at `POST /api/mcp` on the app port, behind the ordinary bearer token. Search tools (`search_documents`, `read_documents`, `count_documents`) share Deep Search's index; plain-access tools (`list_documents`, `get_document`, `list_taxonomy`) read the rows directly. All scoped to the token's user. Costs one embedding request per search, and per read with a `focus`, when embeddings are configured; reads are excerpted text and never call a language model. Read at startup. |
