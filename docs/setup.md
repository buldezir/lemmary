...
| `INGEST_DIR` | unset (off) | A directory inside the container to watch as a consume folder, see [Ingest folder](#ingest-folder). Bind-mount a host folder there (`docker-compose.yml` carries the line commented out). Read at startup; the owner, scan interval and delete-after-consume switch live in Settings → Ingest. Costs one recursive directory walk per interval plus, with deleting off, one SHA-256 of each file the process has not classified yet. |
...
2. Open **Settings** in the nav (shown when `/api/app/me` reports `is_admin`). It has one tab per section, each on its own path and each saving only its own fields: **Appearance** (`/settings`), **AI** (`/settings/ai`), **Processing** (`/settings/processing`), **Worker** (`/settings/worker`), **Duplicates** (`/settings/duplicates`) and, when `INGEST_DIR` is set, **Ingest** (`/settings/ingest`). On the AI tab, add providers, then bind OCR, General AI and optionally the Advanced model to a provider and model — see [Binding models in Settings](/ai_providers#binding-models-in-settings). Changes hot-reload the in-process clients (no restart).
...
With `INGEST_DIR` set, a cron walks that directory and turns every storable file (the same types the upload accepts: PDF, JPEG, PNG, WebP, TXT, CSV, DOCX, XLSX) into a document, exactly as if it had been uploaded: the file is hashed, measured against the instance limits, and queued for the full pipeline. The walk is recursive, and the folders between the root and the file become its tags — `Taxes/2024/invoice.pdf` arrives tagged **Taxes** and **2024**, reusing an existing tag whose name matches case-insensitively and creating the ones that do not exist yet. Dot-files and dot-folders, symlinks, empty files and anything not storable are ignored; a file modified in the last 30 seconds waits for the next scan so nothing half-written is picked up.

The **Ingest** tab (`/settings/ingest`) holds the three settings, all tenant-owned:
...
- **Scan every (minutes)** — default 5. Saving re-schedules the `dir_ingest` cron (visible in PocketBase Admin → Settings → Crons), so a change applies without a restart. Accepted are 1–59 minutes, or whole hours up to 1440 (once a day); anything else is refused, since a cron expression cannot say it.
- **Delete the original file after it is consumed** — off by default. Off, files stay in place and the checksum duplicate check is what keeps them from being imported twice: a file whose content is already in the library is skipped, and remembered as skipped so it is not re-read every scan; that memory lasts until the process restarts, the file changes, or the owner or this switch is changed. Files above the 47 MB document cap and symlinks are never read. On, a file is removed once its document exists, and a file that turns out to be a duplicate is removed as well. A file the pipeline refuses (wrong content for its extension, over a per-file limit) is never deleted; it is logged and skipped. Reaching an instance-wide limit stops the scan until the next interval.
