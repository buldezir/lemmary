...
func TestIngestDirFieldsRoundTripAndAreIdempotent(t *testing.T) {
	...
	if _, err := app.FindCollectionByNameOrId("ingest_files"); err != nil {
		t.Fatalf("ingest_files collection: %v", err)
	}
	...
	settings.Set("ingest_dir_interval_min", 10)
	...
	if reloaded.GetString("ingest_dir_owner") != "user00000000001" ||
		reloaded.GetInt("ingest_dir_interval_min") != 10 ||
