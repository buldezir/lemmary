...
//
// It exists only for the paperless-ngx API, which owns the column name, and
// nothing else reads it: swift-paperless polls `GET /api/tasks/?acknowledged=
// false` and POSTs `/api/acknowledge_tasks/` to clear the ones it has shown, and
// without somewhere to record that the same finished uploads came back on every
// poll forever.
const ngxAcknowledgedField = "ngx_acknowledged"

// Jobs also gain the client-facing id every addressable collection carries: an
// acknowledge names tasks by the id the task list reported.
//
// Unique across the install rather than per account, because a job has no owner
// column: it belongs to one only through its document. Reads stay scoped to the
// caller by joining that document.
func init() {
