...
// produced by one model must not be replayed to another as though the answers in
// it were its own. Empty means the Settings binding.
//
// Text rather than a relation to ai_providers: see the field comment in
// internal/chat/collection.go, the definition a fresh install uses.
func init() {
