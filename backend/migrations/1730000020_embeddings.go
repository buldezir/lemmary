...
// index can be sized before any embedding call is made. Read-only in the
// Settings API for the same reason: 1536 next to a 3072-dimension model would
// produce an index that silently drops every vector.
//
// The tables are created here rather than lazily on first use so that a migrated
// instance has them whether or not embeddings are ever turned on.
func init() {
