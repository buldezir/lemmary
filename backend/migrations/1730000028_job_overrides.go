...
//	 "extract":{...},"embedding":{...}}
//
// which is worth a column of its own precisely because the set of bindings keeps
// growing: app_settings has taken four migrations to reach six pairs.
//
// Empty or absent means the Settings bindings. The column is validated in the
// processing_jobs create hook rather than in the reprocess handler, because the
// single-document reprocess form writes a job straight through the collection
// API.
func init() {
