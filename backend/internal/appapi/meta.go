...
func handleGetMeta(app core.App, rt *config.Runtime) func(*core.RequestEvent) error {
	return func(e *core.RequestEvent) error {
		...
		return writeJSON(e, http.StatusOK, map[string]any{
			...
			// status the document list defaults to, and where an upload
			// lands. Only an admin can change it, but everyone has to know
			// it, and the SPA reads meta before it knows whose session it is.
