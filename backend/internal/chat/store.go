...
func AppendTurn(app core.App, userID, sessionID string, turn Turn) (*core.Record, error) {
	...
	err := app.RunInTransaction(func(txApp core.App) error {
		...
		if err := saveMessage(txApp, session.Id, next, RoleUser, turn.UserContent, turn.RunID, nil); err != nil {
			return err
		}
		if err := saveMessage(txApp, session.Id, next+1, RoleAssistant, turn.AssistantContent, turn.RunID, turn.Documents); err != nil {
			return err
		}
...
func saveMessage(app core.App, sessionID string, seq int, role, content, runID string, hits []ai.DocumentHit) error {
