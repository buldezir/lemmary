	...
	for batch := range slices.Chunk(uids, fetchBatch) {
		...
		for _, msg := range msgs {
			...
			if !msg.InternalDate.Before(since) {
				parts = attachments(msg.BodyStructure, skip)
			}
			got := consumed
			if len(parts) > 0 {
...
// message; a mail body without one is not a document. Images inside a
// multipart/related are the HTML body's own (logos, icons) and never count.
func attachments(bs imap.BodyStructure, skip map[string]bool) []part {
	return collect(bs, nil, false, skip, nil)
}
...
func collect(bs imap.BodyStructure, path []int, related bool, skip map[string]bool, out []part) []part {
	switch node := bs.(type) {
	case *imap.BodyStructureMultiPart:
		related = strings.EqualFold(node.Subtype, "related")
		for i, child := range node.Children {
			out = collect(child, append(slices.Clone(path), i+1), related, skip, out)
		}
	case *imap.BodyStructureSinglePart:
		...
		if name := node.Filename(); name != "" {
			...
			if zipimport.Storable(ext) && !skip[ext] && !(related && strings.EqualFold(node.Type, "image")) {
		...
		if _, multi := inner.(*imap.BodyStructureMultiPart); multi {
			return collect(inner, path, false, skip, out)
		}
		return collect(inner, append(slices.Clone(path), 1), false, skip, out)
	}
	return out
}
