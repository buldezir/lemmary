...
const (
	...
	// the whole point being that the caller has no session yet, and this app
	...
	// the oldest live entry let an unauthenticated caller push a victim's
	// in-flight handle off the end and destroy a ceremony already under way; a
	// flood can now only stop new ones beginning, and drains within the TTL.
	//
	// Per-IP throttling is deliberately not attempted here: e.RealIP() honours
	// TrustedProxy, which this app leaves unset, so behind a reverse proxy every
	// request reports the proxy's address. The operator's tool is PocketBase's
	// own rate limiter, which composes correctly once TrustedProxy is set.
	maxChallenges = 4096
)
...
// consumed, lives for five minutes, and is only ever read by the process that
// wrote it, so a collection would buy durability nothing here wants. An
// httpOnly cookie was the other candidate, rejected because the whole app API
// is bearer-token and cookieless.
